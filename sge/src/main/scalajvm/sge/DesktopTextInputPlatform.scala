...
/** JVM half of [[DefaultDesktopInput.getTextInput]]: shows a Swing input dialog (ISS-815 clause 3).
  *
  * Mirrors `DefaultLwjgl3Input.getTextInput`: pop a `JOptionPane` input dialog on the AWT event thread, then post the result back to the rendering thread via `postRunnable`. Takes the poster as a
  * function so this helper does not depend on [[DesktopWindow]], keeping the seam small (its Native twin is trivially exercised by a link-proof test).
  */
