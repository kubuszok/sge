...
/** The engine's window and rendering surface: frame timing, display modes, cursors, and the GL bindings a game draws with.
  *
  * Reached through the application context as `Sge().graphics`. [[gl20]] is always present; the higher GL bindings ([[gl30]], [[gl31]], [[gl32]]) are available only when the backend and application
  * config requested them — check the matching `glNNAvailable` flag first. Sizes are typed as [[Pixels]] and frame deltas as [[sge.utils.Seconds]] rather than raw numbers.
  *
  * @note
  *   LibGDX: `com.badlogic.gdx.Graphics`.
  *
...
trait Graphics {
  ...
  /** @return the {@link GL30} instance, empty when OpenGL ES 3.0 was not requested/available (see [[gl30Available]]) */
  def gl30: Nullable[GL30]

  /** @return the {@link GL31} instance, empty when OpenGL ES 3.1 was not requested/available (see [[gl31Available]]) */
  def gl31: Nullable[GL31]

  /** @return the {@link GL32} instance, empty when OpenGL ES 3.2 was not requested/available (see [[gl32Available]]) */
