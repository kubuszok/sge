...
/** A single-line text input field. <p> The preferred height of a text field is the height of the {@link TextFieldStyle#font} and {@link TextFieldStyle#background}. The preferred width of a text field
  ...
  * functionality is provided via the {@link Clipboard} interface. Currently there are two standard implementations, one for the desktop and one for Android. In SGE each platform supplies a working
  * implementation; the Android clipboard is backed by the system `android.content.ClipboardManager` (unlike LibGDX's original Android no-op). <p> The text field allows you to specify an
