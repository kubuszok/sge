...
/** A single-line text input field. <p> The preferred height of a text field is the height of the {@link TextFieldStyle#font} and {@link TextFieldStyle#background}. The preferred width of a text field
  ...
  * functionality is provided via the {@link Clipboard} interface. The upstream javadoc's claim of two implementations with an Android no-op is outdated: LibGDX's own `AndroidClipboard` is already
  * backed by the system `ClipboardManager`. SGE ships three implementations, chosen per platform through the application context — desktop, Android (also `ClipboardManager`-backed), and browser. The
  * browser implementation only writes to the system clipboard; reading it back (paste) works only within the application, a browser security restriction. <p> The text field allows you to specify an
  * {@link OnscreenKeyboard} for displaying a softkeyboard and piping all key events generated by the keyboard to the text field. There are two standard implementations, one for the desktop and one
  * for Android. The desktop keyboard is a no-op, as a softkeyboard is not needed on the desktop. The Android {@link OnscreenKeyboard} implementation will bring up the default IME.
  * @author
  *   mzechner
  * @author
  *   Nathan Sweet
