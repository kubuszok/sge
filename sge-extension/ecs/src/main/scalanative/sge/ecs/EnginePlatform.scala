 ...
 *   Convention: split packages; Scala Native source dir (src/main/scalanative)
...
/** Scala Native fallback for [[Engine.createComponent]] on component types without a registered factory.
  *
  * Scala Native has no runtime reflection, so an unregistered component type resolves to the empty/null-equivalent — the same contract as Ashley's base Engine.createComponent when reflection fails
...
private[ecs] object EnginePlatform {
  ...
  def createComponentReflectively[T <: Component](componentType: Class[T]): Nullable[T] =
    Nullable.empty[T] // Engine.java:70-72 — no Native reflection; unregistered resolves to null-equivalent
  ...
  def createComponentOrNull[T <: Component](componentType: Class[T]): T =
    null.asInstanceOf[T] // no Native reflection
}
