...
class TextraLzmaFontRedSuite extends munit.FunSuite {
  ...
  private given sge.Sge = sge.Sge(StubApplication, new NoopGraphics(), new NoopAudio(), StubFiles, new NoopInput(), StubNet)

  private object StubFiles extends sge.Files {
  ...
  final private class BytesFileHandle(fileName: String, bytes: Array[Byte])(using Sge) extends FileHandleStream(fileName) {
  ...
  final private class CapturingFileHandle(fileName: String)(using Sge) extends FileHandleStream(fileName) {
  ...
  final private class MissingFileHandle(fileName: String)(using Sge) extends FileHandleStream(fileName) {
  ...
  final private class MapFiles(handles: Map[String, FileHandle])(using Sge) extends Files {
