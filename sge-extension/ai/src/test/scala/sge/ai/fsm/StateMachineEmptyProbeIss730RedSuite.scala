...
/** Red suite for ISS-730 clause c3 (wave 2026-07-17-F, territory Z).
  *
  * Probing a state machine that has no current / previous / global state must
  * return the null-equivalent (an empty [[Nullable]]), NOT crash.
  *
  * Original contract (states are plain nullable references):
  *   - com/badlogic/gdx/ai/fsm/DefaultStateMachine.java:90-103 —
  *     `getCurrentState()`, `getGlobalState()`, `getPreviousState()` each simply
  *     `return currentState/globalState/previousState;`, which is `null` on a
  *     freshly built machine (constructor `this(null, null, null)`, lines 42-44).
  *   - com/badlogic/gdx/ai/fsm/StackStateMachine.java `getCurrentState()` returns
  *     the (null) current state, and `getPreviousState()` returns `null` when the
  *     stack is empty in the original's null-based contract.
  *
  * The port force-unwraps: `DefaultStateMachine.getCurrentState = currentState.get`
  * (DefaultStateMachine.scala:72,74,76) and `StackStateMachine.getPreviousState`
  * throws `NullPointerException` explicitly (StackStateMachine.scala:63-68). Both
  * therefore CRASH on an empty machine where the original returns `null`.
  *
  * Faithful contract under the project null-mapping: these accessors should be
  * typed `Nullable[S]` and return `Nullable.empty` when unset (a signature change
  * the implementer must make on the `StateMachine` trait and its impls). This
  * suite is written as a RUNTIME red — it only asserts "must not throw" — because
  * that no-crash assertion compiles against the current `: S` signature yet is
  * satisfiable ONLY by returning the empty null-equivalent (there is no non-empty
  * `S` to hand back on an empty machine, and `orNull` is forbidden by project
  * rules). See the reproducer report for the exact `Nullable[S]` signature spec.
  *
...
class StateMachineEmptyProbeIss730RedSuite extends munit.FunSuite {
  ...
  private final class NoopState extends State[String] {
