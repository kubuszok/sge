...
/** Injected — the reflection-free stand-in for what `DefaultBehaviorTreeReader` asked the JVM at run time (`ClassReflection.newInstance/getAnnotation/getField` become [[TaskRegistry.newTask]],
  * [[TaskRegistry.metaOf]], [[TaskRegistry.fieldOf]]); five reader method bodies substitute onto this table (`MethodBodyTransform`, `GdxAiPolicy`). Closed where java's mechanism is open: an
...
/** The reflection-free task table `DefaultBehaviorTreeReader` resolves against.
  *
  * Mutable and process-global, because the thing it replaces is too: java's answer came from the classpath, which is one per JVM. Register before parsing.
  */
object TaskRegistry {

  /** one `@TaskAttribute` slot, as `findMetadata` needs to see it. */
  final class Attr private[utils] (val name: String, val fieldName: String, val required: Boolean, val field: TaskField)

  /** one task class's `@TaskConstraint` plus every `@TaskAttribute` in its hierarchy — what java built out of `getAnnotation` and `getFields`.
    */
  final class Meta private[utils] (val minChildren: Int, val maxChildren: Int, val attributes: Array[Attr])
  ...
  /** java's `getAnnotation(clazz, TaskConstraint.class)` + `getFields` + `getDeclaredAnnotation( TaskAttribute.class)`, over the registration table.
    */
  def metaOf(clazz: java.lang.Class[?]): Meta = {
    ...
    if (found) { new Meta(min, max, attrs.values.toArray) }
    else { null }
  }

  /** java's `ClassReflection.getField(clazz, fieldName)`, restricted to the slots this port knows how to coerce and store. `null` where there is none — `getField`'s substituted body turns that into
    * the `GdxRuntimeException` java raised for a `ReflectionException`.
    */
  ...
  /** the `@TaskConstraint` of a task class, by its runtime class name. Inherited by every subclass with no constraint of its own, exactly as java's `@Inherited` annotation was.
    */
  def constrain(className: String, minChildren: Int, maxChildren: Int): Unit =
    constraints.put(className, (minChildren, maxChildren))

  /** the factory `ClassReflection.newInstance` stood for. Registered under EVERY name a `.btree` file may spell the class as — see [[registerBuiltins]] on why there are two per built-in.
    */
  ...
  def attribute(
    ...
    set:       (java.lang.Object, java.lang.Object) => Unit
  ): Unit = {
  ...
  /** `castValue`'s two `Distribution` branches (`:428` for a number, `:442` for a string), which are one function of the target distribution type. The `"constant," + n` spelling is java's own.
    */
  def asDistribution[T <: sge.ai.utils.random.Distribution](
    value:    java.lang.Object,
    tpe:      java.lang.Class[T],
  ...
  /** `castValue`'s enum branch (`:446`) — the constant whose `name()` equals the string, ignoring case, and `null` where none does. The constants are handed in rather than read off the class, because
    * `Class#getEnumConstants` is reflection and the emitted enum is a sealed hierarchy the JVM does not know is one.
    */
  def asEnum[T <: java.lang.Object](value: java.lang.Object, constants: Array[T], nameOf: T => String): java.lang.Object = value match {
  ...
  /** Register everything `DEFAULT_IMPORTS` names, plus the six `@TaskConstraint` sites the whole `btree` hierarchy has.
    */
  private def registerBuiltins(): Unit = {
    ...
    attribute(
      ...
      (t, v) => t.asInstanceOf[sge.ai.btree.leaf.Wait[java.lang.Object]].seconds = v.asInstanceOf[sge.ai.utils.random.FloatDistribution]
    )
    ...
    attribute(
      ...
      "String",
      (v, _) => asString(v),
      (t, v) => t.asInstanceOf[sge.ai.btree.decorator.Include[java.lang.Object]].subtree = v.asInstanceOf[java.lang.String]
    )
    ...
    attribute(
      ...
      "boolean",
      (v, _) => asBoolean(v),
      (t, v) => t.asInstanceOf[sge.ai.btree.decorator.Include[java.lang.Object]].`lazy` = v.asInstanceOf[java.lang.Boolean].booleanValue()
    )
    ...
    attribute(
      ...
      (t, v) => t.asInstanceOf[sge.ai.btree.decorator.Repeat[java.lang.Object]].times = v.asInstanceOf[sge.ai.utils.random.IntegerDistribution]
    )
    ...
    attribute(
      ...
      "FloatDistribution",
      (v, a) => asDistribution(v, classOf[sge.ai.utils.random.FloatDistribution], a),
      (t, v) => t.asInstanceOf[sge.ai.btree.decorator.Random[java.lang.Object]].success$shadow = v.asInstanceOf[sge.ai.utils.random.FloatDistribution]
    )
    ...
    attribute(
      ...
      "String",
      (v, _) => asString(v),
      (t, v) => t.asInstanceOf[sge.ai.btree.decorator.SemaphoreGuard[java.lang.Object]].name = v.asInstanceOf[java.lang.String]
    )
    ...
    attribute(
      ...
      (t, v) => t.asInstanceOf[sge.ai.btree.branch.Parallel[java.lang.Object]].policy = v.asInstanceOf[sge.ai.btree.branch.Parallel.Policy]
    )
    ...
    attribute(
      ...
      (t, v) => t.asInstanceOf[sge.ai.btree.branch.Parallel[java.lang.Object]].orchestrator = v.asInstanceOf[sge.ai.btree.branch.Parallel.Orchestrator]
    )
  }

  /** register one built-in under BOTH the port's emitted FQCN and gdx-ai's upstream one — see [[registerBuiltins]].
    */
