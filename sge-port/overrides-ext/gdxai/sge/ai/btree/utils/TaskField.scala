...
/** INJECTED — the REDIRECT TARGET for `com.badlogic.gdx.utils.reflect.Field` (`TypeRedirectTransform`, see `GdxAiPolicy`): the libGDX base drops that type outright since runtime reflection is
  * unportable, so `DefaultBehaviorTreeReader`'s three `reflect.Field` signatures re-point here instead of being cut. It carries what a `Field` gave the parser — coercing a value to the field's static
  * type and storing it — as closures set at registration.
  */
...
final class TaskField private[utils] (
  ...
  /** java's `castValue(field, value)` for this one field: the coerced value, or `null` where the parsed value is not assignable to it. `null` and not an `Option` — this is java's own protocol and
    * `setField` tests it exactly as java did.
    */
