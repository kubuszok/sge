/** The PORT half of USL's conformance oracle. Its twin is `OracleJava.java` beside it, and `just usl-oracle-measure` runs both and diffs the transcripts line for line.
  */
object Oracle {

  def main(args: Array[String]): Unit = {
    val stylesDir = new java.io.File(args(0))
    val knownGood = new String(java.nio.file.Files.readAllBytes(java.nio.file.Paths.get(args(1))), java.nio.charset.StandardCharsets.UTF_8)

    val fixtures = Option(stylesDir.listFiles((_, n) => n.endsWith(".usl"))).getOrElse(Array.empty[java.io.File])
    if fixtures.isEmpty then throw new IllegalStateException("no .usl fixtures under " + stylesDir)
