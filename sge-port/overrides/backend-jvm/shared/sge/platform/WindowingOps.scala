...
/** Windowing system operations (GLFW/SDL3). Defines the FFI contract for creating windows, handling input, cursors, and clipboard.
  ...
  *
  * The method set is what the desktop backend files call; a new backend need adds a method here and an implementation per platform.
  */
private[sge] trait WindowingOps {
  ...
  /** Processes every event the windowing system has queued (input, resize, etc.). */
  def pollEvents(): Unit
