...
class Json {
  ...
  /** THE SWAP POINT: bind the Kindlings Jsoniter/UBJson codec here and delegate the reflective paths to it. Until then they fail loudly rather than pretending to decode.
    */
  ...
  /** Java's parameter is `Serializer<T>`; ours is `Serializer[?]`, for the same reason `read` returns `Object` above: libGDX registers RAW `new ReadOnlySerializer() {…}` instances, and a raw
    * anonymous class's expected type does not propagate into its parent, so it infers `Nothing` and `Serializer[Nothing]` matches no `Serializer[X]`. javac accepted it unchecked too, and the map
    * below is untyped anyway, so the erased registration is the faithful one.
    */
  ...
  /** `protected boolean ignoreUnknownField (Class type, String fieldName)` — libgdx's Json calls it from `readFields`, and `Skin` overrides it. Absent here, that override compiled to nothing.
    */
