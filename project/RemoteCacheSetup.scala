...
/** BuildBuddy remote-cache wiring for the sbt 2 Bazel-compatible gRPC cache (ISS-792).
  *
  * sbt 2 speaks the Bazel remote-cache protocol natively (via `addRemoteCachePlugin` in
  * project/plugins.sbt); the endpoint + auth header settings consumed in build.sbt are
  * `Global / remoteCache` and `Global / remoteCacheHeaders`
  * (https://www.scala-sbt.org/2.x/docs/en/reference/remote-cache-setup.html). BuildBuddy
  * authenticates via the `x-buildbuddy-api-key` gRPC header
  ...
  *
  * When no key is found the remote cache stays OFF (`remoteCache = None`) and the build
  * behaves exactly as before — contributors and CI forks need no setup. Set
  * `SGE_REMOTE_CACHE=0` (or `off`/`false`) to force-disable even when a key is present
  * (useful for debugging cache behaviour). The key is only ever placed in the in-memory
  * gRPC header value — never logged and never written to disk by this wiring, and
  * `show Global / remoteCache` prints only the endpoint. (Note `remoteCacheHeaders`
  * necessarily contains the key: avoid `show Global / remoteCacheHeaders` in logs.)
  *
  * Read/write policy: BuildBuddy's org API key is read-write, and every environment that
  * has it (local dev, master CI, same-repo PR CI) both reads and writes the cache. Fork
  * PRs have no secret, so they cannot read or write. See
  * docs/reviews/ci-cache-investigation-2026-07-16.md for the measurements behind this.
  */
object RemoteCacheSetup {
  ...
  /** The resolved API key, if any. Deliberately private: nothing outside the two derived
    * values below should ever see (let alone print) it.
    */
