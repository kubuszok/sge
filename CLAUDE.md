...
- `sge-port/overrides/<step>/` — the hand-written files the policy injects by path, only those sge's
  own tree does not hold (14: the per-row async executor, `Align`, the `Sge` context, `Timer`,
  `TextFormatter`, the reflection-free stand-ins; each one's reason is an ISS-894..901 row, or ISS-871). They are covenanted and scanned like every ported file (`sge-port/overrides` is
...
- Android: FILEHANDLE_TYPES, TOUCH_DISPATCH, LIFECYCLE (CLIPBOARD lifted 2026-07-18 and JSON_XML lifted 2026-09-16 — both pass on the x86_64 emulator frame phase, now required checks; ISS-701/ISS-694)
